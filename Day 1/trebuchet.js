...
const trebuchet = (data) => {
	...
	for (let i in data) {
		...
		for (let j = 0; j < data[i].length; j++) {
			if (firstNum === null && !isNaN(data[i][j])) firstNum = data[i][j]
			if (firstNum !== null && !isNaN(data[i][j])) lastNum = data[i][j]
		}
		const combinedNum = firstNum + lastNum
		calibratedValue = calibratedValue + Number(combinedNum)
...
const trebuchet2 = (data) => {
	...
	for (let i = 0; i < data.length; i++) {
		for (let num in wordToNum) {
			while (data[i].includes(num)) {
				data[i] = data[i].replace(num, wordToNum[num])
			}
		}
	}
